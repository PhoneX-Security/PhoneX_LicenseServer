...
return [
    ...
    */
    ...
    'permissions' => function (Manager $manager, Lock $caller) {
        ...
        // We only want to set permissions beforehand if$manager->role('admin')->allow('route', 'logs'); we're using the array driver.
        ...
        if ($manager->getDriver() instanceof ArrayDriver) {
            ...
            $manager->role('admin')->allow('route', 'logs');
            $manager->role('admin')->allow('route', 'adminer');
